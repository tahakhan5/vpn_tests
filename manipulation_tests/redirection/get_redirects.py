...
def get_redirects(results_dir, host):
    ...
    subprocess.call(['mkdir', results_dir+host])

    # Write all network requests
    with open(results_dir+host+"/request_logs.json", 'w') as outfile:
        for x in sorted_nr:
            json.dump(x, outfile)
            outfile.write("\n")
    ...
    with open(results_dir+host+"/page_redirs.txt", 'w') as outfile:
    ...
    with open(results_dir+host+"/header_redirs.txt", 'w') as outfile:
...
def main():
    ...
    for host in web_hosts:
        try:
            get_redirects(results_dir, host)
        except:
